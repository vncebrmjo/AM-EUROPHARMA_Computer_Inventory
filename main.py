...
def main_page():
    if g.user:
        cursor.execute("SELECT * FROM INV_Comp")
        get_page = cursor.fetchall()
        return render_template(
            "main_page.html", get_page=get_page, user=session["Username"]
        )
...
def update_inventory():
    if g.user:
        ...
        cursor = connect.cursor()
        cursor.execute("SELECT * FROM INV_Comp WHERE ID = ?", (id,))
...
def update_department():
    if request.method == "POST":
        ...
        cursor.execute(
            "INSERT INTO INV_reportlogs (id,  date_modified, field_modified, old_data, new_data) "
            "VALUES ( ?, ?, ?, ?, ?)",
            (id, dept_date, field, old_department, new_department),
        )

        cursor.execute(
            "UPDATE INV_Comp SET department = ? WHERE ID = ?", (new_department, id)
        )
...
def update_username():
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM INV_Comp WHERE Username = ?", (new_username,))
        row = cursor.fetchone()
        if row:
            error = "The Username already exists"
            cursor.execute("SELECT * FROM INV_Comp")
            get_page = cursor.fetchall()
            # return redirect(url_for('update_user', error=error))
            return render_template("update_user.html", error=error, get_page=get_page)
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Username = ? WHERE ID = ?", (new_username, id)
        )
        msg = "The username has been updated successfully"

        connect.commit()

        # Fetch all records to display after submission
        cursor.execute("SELECT * FROM INV_Comp")
        get_page = cursor.fetchall()
        ...
        #                             user_date=user_date, get_page=get_page))
        return render_template(
            "update_user.html",
    ...
    if g.user:
        ...
        get_page = cursor.fetchall()
        return render_template(
            "update_user.html", get_page=get_page, user=session["Username"]
        )
    return redirect(url_for("login"))
...
def update_computername():
    if request.method == "POST":
        ...
        cursor.execute(
            "SELECT * FROM INV_Comp WHERE Computer_Name = ?", (new_computername,)
        )
        row = cursor.fetchone()
        if row:
            error1 = "The Computer name already exists"
            cursor.execute("SELECT * FROM INV_Comp")
            get_page = cursor.fetchall()
            # return redirect(url_for('update_user', error=error))
            return render_template("update_user.html", error1=error1, get_page=get_page)
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Computer_Name = ? WHERE ID = ?", (new_computername, id)
        )
        msg = "The Data has been updated successfully"
        ...
        return render_template(
            "update_user.html",
            new_computername=new_computername,
            old_computername=old_computername,
            field=field,
            id=id,
            msg=msg,
            com_date=com_date,
...
def update_ip():
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM INV_Comp WHERE IP = ?", (new_ip,))
        row = cursor.fetchone()
        if row:
            error233 = "The IP already exists"
            cursor.execute("SELECT * FROM INV_Comp")
            get_page = cursor.fetchall()
            # return redirect(url_for('update_user', error=error))
            return render_template(
                "update_user.html", error233=error233, get_page=get_page
            )
        ...
        cursor.execute("UPDATE INV_Comp SET IP = ? WHERE ID = ?", (new_ip, id))
        msg = "The Data has been updated successfully"
...
def update_asset_tag():
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM INV_Comp WHERE Asset_Tag = ?", (new_asset_tag))
        row = cursor.fetchone()
        if row:
            error3 = "The Asset Tag already exists"
            cursor.execute("SELECT * FROM INV_Comp")
            get_page = cursor.fetchall()
            # return redirect(url_for('update_user', error=error))
            return render_template("update_user.html", error3=error3, get_page=get_page)
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Asset_Tag = ? WHERE ID = ?", (new_asset_tag, id)
        )
        msg = "The Data has been updated successfully"
...
def update_serial_number():
    if request.method == "POST":
        ...
        cursor.execute(
            "SELECT * FROM INV_Comp WHERE Serial_Number = ?", (new_serial_number)
        )
        row = cursor.fetchone()
        if row:
            error4444 = "The Serial Number already exists"
            cursor.execute("SELECT * FROM INV_Comp")
            get_page = cursor.fetchall()
            # return redirect(url_for('update_user', error=error))
            return render_template(
                "update_user.html", error4444=error4444, get_page=get_page
            )
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Serial_Number = ? WHERE ID = ?",
...
def update_processor():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Processor = ? WHERE ID = ?", (new_processor, id)
        )
        msg = "The data has been updated successfully"
...
def update_os():
    if request.method == "POST":
        ...
        cursor.execute("UPDATE INV_Comp SET OS = ? WHERE ID = ?", (new_os, id))
        msg = "The data has been updated successfully"
...
def update_mobo():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET MotherBoard = ? WHERE ID = ?", (new_mobo, id)
        )
        msg = "The data has been updated successfully"
...
def update_ram():
    if request.method == "POST":
        ...
        cursor.execute("UPDATE INV_Comp SET RAM = ? WHERE ID = ?", (new_ram, id))
        msg = "The data has been updated successfully"
...
def update_storage():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Storage = ? WHERE ID = ?", (new_storage, id)
        )
        msg = "The data has been updated successfully"
...
def update_ps():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Power_Supply = ? WHERE ID = ?", (new_ps, id)
        )
        msg = "The data has been updated successfully"
...
def update_msoffice():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET MS_Office = ? WHERE ID = ?", (new_msoffice, id)
        )
        msg = "The data has been updated successfully"
        ...
        return render_template(
            "update_inventory.html",
            new_msoffice=new_msoffice,
            old_msoffice=old_msoffice,
            field=field,
            id=id,
            msg=msg,
            ms_date=ms_date,
...
def update_comtag():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Computer_Tag = ? WHERE ID = ?", (new_comtag, id)
        )
        msg = "The data has been updated successfully"
        ...
        return render_template(
            "update_inventory.html",
            new_comtag=new_comtag,
            old_comtag=old_comtag,
            field=field,
            id=id,
            msg=msg,
            comtag_date=comtag_date,
        )
...
def update_nettag():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Network_Tag = ? WHERE ID = ?", (new_nettag, id)
        )
        msg = "The data has been updated successfully"
        ...
        return render_template(
            "update_inventory.html",
            new_nettag=new_nettag,
            old_nettag=old_nettag,
            field=field,
            id=id,
            msg=msg,
            nettag_date=nettag_date,
        )
...
def update_eset():
    if request.method == "POST":
        ...
        cursor.execute("UPDATE INV_Comp SET ESET = ? WHERE ID = ?", (new_eset, id))
        msg = "The data has been updated successfully"
...
def update_ups():
    if request.method == "POST":
        ...
        cursor.execute("UPDATE INV_Comp SET UPS = ? WHERE ID = ?", (new_ups, id))
        msg = "The data has been updated successfully"
...
def update_opstat():
    if request.method == "POST":
        ...
        cursor.execute(
            "UPDATE INV_Comp SET Operational_Status = ? WHERE ID = ?", (new_opstat, id)
        )
        msg = "The data has been updated successfully"
        ...
        return render_template(
            "update_inventory.html",
            new_opstat=new_opstat,
            old_opstat=old_opstat,
            field=field,
            id=id,
            msg=msg,
...
def add_inventory2():
    if g.user:
        if request.method == "POST":
            ...
            cursor.execute("SELECT * FROM INV_Comp WHERE Username = ?", user)
            row = cursor.fetchone()
            if row:
                error = "The Username already exists"
        ...
        if request.method == "POST":
            ...
            cursor.execute(
                "SELECT * FROM INV_Comp WHERE COMPUTER_NAME = ?", computer_name
            )
            row = cursor.fetchone()
            if row:
        ...
        if request.method == "POST":
            ...
            cursor.execute("SELECT * FROM INV_Comp WHERE IP = ?", ip)
            row = cursor.fetchone()
            if row:
        ...
        if request.method == "POST":
            ...
            cursor.execute("SELECT * FROM INV_Comp WHERE Asset_Tag = ?", asset_tag)
            row = cursor.fetchone()
            if row:
        ...
        if request.method == "POST":
            ...
            cursor.execute(
                "SELECT * FROM INV_Comp WHERE Serial_Number = ?", serial_number
            )
            row = cursor.fetchone()
            if row:
        ...
        if request.method == "POST":
            processor_1 = request.form["processor_1"]
            session["processor_1"] = processor_1
            processor_2 = request.form["processor_2"]
            session["processor_2"] = processor_2
        ...
        if request.method == "POST":
            ...
            cursor.execute(
                "INSERT INTO INV_Comp (DEPARTMENT, USERNAME, COMPUTER_NAME, IP, PROCESSOR, MOTHERBOARD, POWER_SUPPLY, RAM, STORAGE, OS, MS_OFFICE, ASSET_TAG, COMPUTER_TAG, NETWORK_TAG, UPS, SERIAL_NUMBER, OPERATIONAL_STATUS, ESET) "
                "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    department,
                    user,
...
def Report_Logs():
    if g.user:
        cursor.execute("SELECT * FROM INV_reportlogs")
        get_page = cursor.fetchall()

        return render_template(
            "Report_Logs.html", get_page=get_page, user=session["Username"]
        )
    return redirect(url_for("login"))
...
def login():
    if request.method == "POST":
        ...
        cursor.execute(
            "SELECT * FROM INV_login WHERE Username = ? AND Password = ?",
            (username, password),
        )
